...
public class ReflectionService {
    ...
    public ReflectionResponse getReflectionById(UUID id) {
        ...
        if (!hasAccessToReflection(reflection, currentUser)) {
            throw new UnauthorizedReflectionAccessException("You don't have permission to view this reflection");
        }
    ...
    public ReflectionResponse updateReflection(UUID id, UpdateReflectionRequest request) {
        ...
        if (!isAuthorOrAdmin(reflection, currentUser)) {
            throw new UnauthorizedReflectionAccessException("You don't have permission to update this reflection");
        }
    ...
    public void deleteReflection(UUID id) {
        ...
        // Check if user is author or admin
        if (!isAuthorOrAdmin(reflection, currentUser)) {
            throw new UnauthorizedReflectionAccessException("You don't have permission to delete this reflection");
        }
    ...
    public List<ReflectionResponse> getAccessibleReflections() {
        User currentUser = userService.getCurrentUser();
        List<Reflection> reflections = reflectionRepository.findReflectionsAccessibleToUser(currentUser.getId());
    ...
    public List<ReflectionResponse> getHouseholdReflections() {
        ...
        List<Reflection> reflections = reflectionRepository.findByHouseholdId(currentUser.getActiveHousehold().getId());
        return reflections.stream()
                .map(this::toResponse)
                .collect(Collectors.toList());
    }
