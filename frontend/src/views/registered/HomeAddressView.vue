...
import { MapPin, Mail, Building, Globe } from 'lucide-vue-next';
...
const geocodeAddress = async (address: string): Promise<{ lat: number, lng: number } | null> => {
  try {
    const fullAddress = `${address}, ${formValues.value.postalCode} ${formValues.value.city}, ${formValues.value.country}`;
...
const onSubmit = handleSubmit(async (values) => {
  ...
  try {
    ...
    if (coords) {
      ...
      alert('Kunne ikke finne koordinater for denne adressen. Vennligst sjekk adressen og prøv igjen.');
      loading.value = false;
    }
  } catch (error) {
...
<template>
  <div class="home-address-container my-10">
    <Card class="w-full max-w-lg mx-auto shadow-lg">
      <CardHeader>
        <CardTitle class="text-2xl font-bold text-center">Adresse</CardTitle>
        <CardDescription class="text-center">Fyll inn adresseinformasjon</CardDescription>
      </CardHeader>
