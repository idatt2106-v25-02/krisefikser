...
const formTitle = computed(() =>
  editingReflection.value ? 'Rediger Refleksjon' : 'Skriv en ny refleksjon',
)
const formDescription = computed(() =>
  editingReflection.value
    ? 'Gjør endringer i refleksjonen din her.'
    : 'Del dine tanker og erfaringer knyttet til denne hendelsen.',
)
...
<template>
  <div class="container mx-auto px-4 py-8 max-w-6xl">
    ...
    <div
      v-if="eventLoading"
      class="bg-white rounded-lg shadow-lg p-8 flex flex-col items-center justify-center py-16 mt-6"
    >
      <div
    ...
    <div v-if="event" class="mt-6">
        ...
        <div class="p-6">
          ...
          <div class="flex flex-wrap gap-2 mb-6">
            ...
            >
              ...
              {{
                event.status === EventResponseStatus.FINISHED
                  ? 'Avsluttet: '
                  : event.status === EventResponseStatus.ONGOING
                    ? 'Startet: '
                    : 'Starter: '
              }}
              {{
                formatDate(
                  event.status === EventResponseStatus.FINISHED ? event.endTime : event.startTime,
                )
              }}
            </span>
          </div>
      ...
      <!-- Reflections Section -->
      <div
        v-if="event && event.status === EventResponseStatus.FINISHED"
      ...
      >
        <div class="flex justify-between items-center mb-6">
          ...
          <Button @click="openNewReflectionDialog" class="bg-blue-500 hover:bg-blue-600 text-white">
        ...
        <div v-if="reflectionsLoading" class="flex justify-center items-center py-8">
          <div
            class="inline-block animate-spin rounded-full h-8 w-8 border-4 border-b-blue-500 border-blue-100"
          ></div>
        </div>
                ...
                <span
                  ...
                  :class="{
                    'bg-green-100 text-green-800':
                      reflection.visibility === ReflectionResponseVisibility.PUBLIC,
                    'bg-blue-100 text-blue-800':
                      reflection.visibility === ReflectionResponseVisibility.HOUSEHOLD,
                    'bg-gray-100 text-gray-800':
                      reflection.visibility === ReflectionResponseVisibility.PRIVATE,
                  }"
                >
