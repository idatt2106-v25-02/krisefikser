...
<template>
  <div class="fixed bottom-20 right-5 z-50">
    <button
      ...
      class="bg-blue-500 hover:bg-blue-600 text-white p-3 rounded-full shadow-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 group relative transition-all"
      :class="{ 'animate-pulse': isReading }"
      :aria-label="isReading ? 'Stopp lesing (Alt/Option + R)' : 'Les innholdet på siden (Alt/Option + R)'"
    >
      ...
      <span class="absolute bottom-full right-0 mb-2 px-2 py-1 bg-gray-900 text-white text-xs rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">
        {{ isReading ? 'Stopp lesing (Alt/Option + R)' : 'Les innholdet på siden (Alt/Option + R)' }}
      </span>
    </button>
  </div>
</template>
