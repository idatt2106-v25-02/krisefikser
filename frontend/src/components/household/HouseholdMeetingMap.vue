...
function getDirections(place: MeetingPlace) {
  ...
  const mockRoute = [
    [userPos[0], userPos[1]],
    [userPos[0] + (place.position[0] - userPos[0]) * 0.3, userPos[1] + (place.position[1] - userPos[1]) * 0.3],
    [userPos[0] + (place.position[0] - userPos[0]) * 0.7, userPos[1] + (place.position[1] - userPos[1]) * 0.7],
...
<template>
  <div class="relative w-full  h-[500px] border border-gray-200 rounded-lg overflow-hidden">
    ...
    <div v-if="selectedMeetingPlace" class="absolute bottom-16 left-4 right-4 bg-white p-3 rounded-lg shadow-md max-w-sm">
      <div class="flex justify-between items-start">
        ...
        <button
          ...
          @click="selectedMeetingPlace = null"
        >
    ...
    <div v-if="directionsVisible && selectedMeetingPlace" class="absolute top-4 right-4 bg-white p-3 rounded-lg shadow-md max-w-sm">
      <div class="flex justify-between items-center mb-2">
        <h3 class="font-bold text-gray-800 text-sm">Veibeskrivelse til {{ selectedMeetingPlace.name }}</h3>
        <button
          class="text-gray-500 hover:text-gray-700"
          @click="directionsVisible = false"
        >
