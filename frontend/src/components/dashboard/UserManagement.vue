...
const props = defineProps({
  ...
  // The households data from parent component
  householdsData: {
    type: Array as () => HouseholdResponse[],
    default: () => []
  },
  // Loading states
  isLoadingUsers: {
...
const selectedUser = computed(() => {
  if (!selectedUserId.value || !props.usersData) return null;
  return props.usersData.find(user => user.id === selectedUserId.value) || null;
});
...
const sendAdminInvite = () => {
  ...
  const user = props.usersData?.find(u => u.id === selectedUserId.value);
  if (!user) return;
...
<template>
  <div class="p-6">
    ...
    <div class="bg-white rounded-lg shadow overflow-hidden mb-6">
      ...
      <!-- Loading state -->
      <div v-if="isLoadingUsers && viewMode !== 'households'" class="p-8 text-center">
      ...
      <!-- Household view -->
      <div v-else-if="viewMode === 'households' && showHouseholdTab">
